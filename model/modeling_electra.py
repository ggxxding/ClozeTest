...
class ElectraForCloth(ElectraPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(self, inp, tgt):
        ...
        bsz = ops.size(0)
        opnum = ops.size(1)
        generator_hidden_states = self.electra(articles, attention_mask=articles_mask, return_dict=False)
        generator_sequence_output = generator_hidden_states[0]

        out = self.generator_predictions(generator_sequence_output)
        ...
        out = self.generator_lm_head(out)
