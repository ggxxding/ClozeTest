...
class AlbertModel(AlbertPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            #print((sequence_output, pooled_output) + encoder_outputs[1:])
...
class AlbertForCloth(AlbertPreTrainedModel):
    def __init__(self,config):
        print('AlbertForCloth inited, config:')
        print(config)
    ...
    def forward(self, inp, tgt):
        ...
        bsz = ops.size(0)
        opnum = ops.size(1)
        out, _ = self.albert(articles, attention_mask = articles_mask, return_dict=False)
        #out.shape  [batch, max_article_len, hidden_size]
        question_pos = question_pos.unsqueeze(-1)#[batch, num_ques] -> [batch, num_ques, 1]
        question_pos = question_pos.expand(bsz, opnum, out.size(-1))#[batch, num_ques, hidden_size]
        out = torch.gather(out, 1, question_pos)#[batch, num_ques, hid] 取出out中question位置的隐层参数

        out = self.cls(out)#[batch, num_ques, vocab_size](prediction

        #convert ops to one hot
        out = out.view(bsz, opnum, 1, self.vocab_size)#[batch, num_ques, 1, 30000]
        out = out.expand(bsz, opnum, 4, self.vocab_size)
        #[batch, num_ques, 4, 30000]
        out = torch.gather(out, 3, ops)
        #ops=[batch, num_ques, 4, max_ops_len]=out


        #mask average pooling
        out = out * ops_mask
        out = out.sum(-1)#[batch, num_ques, 4]
        out = out/(ops_mask.sum(-1))#概率是否softmax?
        out = out.view(-1, 4)#[batch*num_ques, 4]
        tgt = tgt.view(-1,)#[batch*num_ques]
        loss = self.loss(out, tgt)

        acc = self.accuracy(out, tgt)
        loss = loss.view(bsz, opnum)#[batch,num_ques]
