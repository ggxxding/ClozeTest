...
import argparse
import random
import data_util_Albert
from data_util_Albert import ClothSample
import numpy as np
import torch
import time
from pytorch_pretrained_bert.optimization import BertAdam
from transformers.modeling_albert import AlbertForCloth
...
def main():
    ...
    train_data = None
    if args.do_train:
        train_data = data_util_Albert.Loader(args.data_dir, data_file['train'], args.cache_size, args.train_batch_size, device)
        num_train_steps = int(
            train_data.data_num / args.train_batch_size / args.gradient_accumulation_steps * args.num_train_epochs)

    # Prepare model
    model = AlbertForCloth.from_pretrained(args.bert_model)
    if args.fp16:
        model.half()
    model.to(device)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        valid_data = data_util_Albert.Loader(args.data_dir, data_file['valid'], args.cache_size, args.eval_batch_size, device)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        test_data = data_util_Albert.Loader(args.data_dir, data_file['test'], args.cache_size, args.eval_batch_size, device)
