...
import argparse
import random
import data_util_electra
from data_util_electra import ClothSample
import numpy as np
import torch
import time
from pytorch_pretrained_bert.optimization import BertAdam
from transformers.modeling_electra import ElectraForCloth
...
def main():
    ...
    for key in data_file.keys():
        data_file[key] = data_file[key] + '-' +args.bert_model[7:]+ '.pt'
    ...
    train_data = None
    if args.do_train:
        train_data = data_util_electra.Loader(args.data_dir, data_file['train'], args.cache_size, args.train_batch_size, device)
        num_train_steps = int(
            train_data.data_num / args.train_batch_size / args.gradient_accumulation_steps * args.num_train_epochs)

    # Prepare model
    model = ElectraForCloth.from_pretrained(args.bert_model)
    if args.fp16:
        model.half()
    model.to(device)
    ...
    print(type(optimizer_grouped_parameters),len(optimizer_grouped_parameters))
        ...
        for _ in range(int(args.num_train_epochs)):
            ...
            for inp, tgt in train_data.data_iter():
                ...
                if (global_step % args.num_log_steps == 0):
                    ...
                    tr_acc = 0
                    nb_tr_examples = 0
        torch.save(model.state_dict(), './data/'+args.bert_model[7:]+'-model.pt')
        print('saved as ./data/'+args.bert_model[7:]+'-model.pt')
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        logging("***** Running evaluation *****")
        logging("  Batch size = {}".format(args.eval_batch_size))
        valid_data = data_util_electra.Loader(args.data_dir, data_file['valid'], args.cache_size, args.eval_batch_size, device)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        test_data = data_util_electra.Loader(args.data_dir, data_file['test'], args.cache_size, args.eval_batch_size, device)
